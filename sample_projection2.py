...
def save_params():
    ...
    if check_nones((state.tvec, state.rvec,
        ...
        np.savetxt("homography_pjcm2.txt", state.H_pj_cm)
    else:
...
def on_key_press_impl(symbol, modifiers):
    ...
    if symbol == pyglet.window.key.Z:
        save_params()
        np.savetxt("modelview_matrix2.txt", modelview_matrix)
        np.savetxt("projection_matrix2.txt", projection_matrix)
...
def obtain_homography_matrix():
    ...
    if get_camera_frame():
        ...
        # f.close()
        # np.savetxt("homography_pjcm2.txt", H)
