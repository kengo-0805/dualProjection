...
TARGET_SCREEN_ID = 2     # プロジェクタのスクリーンID
CAMERA_ID = 1           # カメラID
...
def save_params():
    ...
    if check_nones((state.tvec, state.rvec,
        ...
        np.savetxt("Hpjcm_1.txt", state.H_pj_cm)
    else:
...
def obtain_control_point():
    ...
    if get_camera_frame():
        ...
        # plotter = np.array(Image.open('./align/align_img_c0.png'))
        camera_frame = cv2.imread("./align/align_img_c1.png")
...
def obtain_homography_matrix():
    ...
    if get_camera_frame():
        ...
        # plotter = Plotter(imgarray=state.camera_frame)
        # plotter = np.array(Image.open('./align/align_img_h0.png'))
        camera_frame = cv2.imread("./align/align_img_h1.png")
        camera_frame = cv2.cvtColor(camera_frame, cv2.COLOR_RGB2BGR)
        plotter = Plotter(imgarray=camera_frame)
